...
python merge_lora_low_mem.py \
    ...
    --lora_model /scratch/project_462000319/aralikatte/cpt/scripts/training/models/cpt-sv-mistral-7b-1/checkpoint-1 \
    --output_type huggingface \
    --output_dir /scratch/project_462000319/aralikatte/cpt/scripts/training/models/cpt-sv-mistral-7b-1/checkpoint-1-merged
"""
...
parser = argparse.ArgumentParser(
    description="Script to merge Llama-3-hf with Llama-3-Chinese or Llama-3-Chinese-Instruct LoRA weights"
)
parser.add_argument(
    "--base_model",
    default=None,
    required=True,
    type=str,
    help="Base model path (basically Llama-3-hf)",
)
parser.add_argument(
    "--lora_model",
    default=None,
    required=True,
    type=str,
    help="LoRA model path (Llama-3-Chinese-LoRA, Llama-3-Chinese-Instruct-LoRA)",
)
parser.add_argument(
    "--output_type",
    default="huggingface",
    choices=["huggingface", "pth"],
    type=str,
    help="Output model type can be 'huggingface' (default) or 'pth' format",
)
parser.add_argument(
    "--output_dir",
...
def save_shards(model_sd, num_shards: int, prefix="", verbose=False):
    ...
    with torch.no_grad():
        if num_shards == 1:
            ...
            print(
                f"Saving shard 1 of {num_shards} into {output_dir}/{prefix}consolidated.00.pth"
            )
            ...
            for i, new_state_dict in enumerate(new_state_dicts):
                print(
                    f"Saving shard {i+1} of {num_shards} into {output_dir}/{prefix}consolidated.0{i}.pth"
                )
                torch.save(
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(base_model_path, "pytorch_model.bin")):
        ...
        ckpt_filenames = sorted(
            [
                ...
                if re.match("model-(\d+)-of-(\d+).safetensors", f)
            ]
        )
    ...
    if len(ckpt_filenames) == 0:
        raise FileNotFoundError(
            f"Cannot find base model checkpoints in ${base_model_path}. Please make sure the checkpoints are saved in the HF format."
        )
    ...
    for index, filename in enumerate(ckpt_filenames):
        ...
        if re.match("(.*).safetensors", filename):
            state_dict = safe_load_file(
                os.path.join(base_model_path, filename), device="cpu"
            )
        else:
        ...
        for config in configs:
            if os.path.exists(os.path.join(lora_model_path, config)):
                ...
                if os.path.exists(os.path.join(base_model_path, config)):
                    ...
                    if (
                        config == "pytorch_model.bin.index.json"
                        or config == "model.safetensors.index.json"
                    ):
